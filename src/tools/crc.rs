        ...
        if first_render {
            if let Some(window) = window() {
                ...
                if let Some(doc) = document {
                    ...
                    {
                        meta_tag.set_attribute("content", "This page provides a simple and efficient tool for calculating CRC (Cyclic Redundancy Check) values for data integrity verification and error detection.").unwrap();
                    }
                }
            }
        }
